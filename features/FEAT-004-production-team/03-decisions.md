...
- 화면: `components/production/ProductionLogView.tsx` 안에 "생산일지 조회"/"생산효율" 모드 토글을 추가해, 생산효율 모드에서는 상단에 "시작 기간~종료 기간" 드롭다운 2개를 두고, 선택이 바뀌면 즉시 재조회한다. 기본값은 전체 기간(가장 오래된 것~가장 최근 것).
